...
const CA = (process.env.CA || '0x83d38b519308fe02158f043952b58a6b16a4a30e').toLowerCase();
...
// ---- memoised state (per warm instance) ----
const memo = { pool: null, poolAt: 0, meta: null, supply: null, supplyAt: 0, eth: null, ethAt: 0 };
...
async function pool() {
  ...
  let p;
  if (process.env.POOL) {
    p = { version: process.env.POOL_VERSION || 'v4', pool: process.env.POOL, quote: (process.env.QUOTE || '0x0000000000000000000000000000000000000000').toLowerCase() };
  } else {
    // one-time discovery of the pool id (not the price) — override with POOL/POOL_VERSION/QUOTE env vars to skip entirely
    const j = await (await fetch(`https://api.dexscreener.com/latest/dex/tokens/${CA}`)).json();
    const pairs = (j.pairs || []).filter(x => x.chainId === DEX_CHAIN && x.baseToken.address.toLowerCase() === CA)
      .sort((a, b) => ((b.liquidity && b.liquidity.usd) || 0) - ((a.liquidity && a.liquidity.usd) || 0));
    if (!pairs.length) throw new Error('no pool for this token yet');
    const best = pairs[0];
...
async function meta(p) {
  ...
  if (p.quote !== '0x0000000000000000000000000000000000000000') {
...
module.exports = async (req, res) => {
  ...
  try {
    ...
    const [pq, sup] = await Promise.all([priceInQuote(p, m), supply(m.dec)]);
    let usdPerQuote;
    if (m.qSym === 'ETH' || m.qSym === 'WETH') usdPerQuote = await ethUsd();
    else if (STABLES.has(m.qSym)) usdPerQuote = 1;
    else return send(503, { error: 'unsupported quote token ' + m.qSym });
    const price = pq * usdPerQuote;
    send(200, { mc: price * sup, price, priceQuote: pq, quote: m.qSym, usdPerQuote, supply: sup, symbol: m.sym, pool: p.pool, version: p.version, source: 'alchemy:' + NET, ts: Date.now() }, true);
  } catch (e) {
